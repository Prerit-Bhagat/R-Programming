...
The previous loop is a bit dangerous because there's no guarantee it will stop. Better to set a hard limit on the number of iterations (e.g.using a for loop)
...
result1 <- my_function(5, 20)  # Result is 25

# Calling the function with only one argument
result2 <- my_function(5)  # Result is 15 (y defaults to 10)
...
# Only providing a value for 'a'
result <- my_function(5)  # Result is 5 + 2 + 3 = 10
...
result1 <- my_function(5, 3)  # Result is 8
result2 <- my_function(5)     # Result is 5 (b is missing, so defaults to 0)
...
# Calculate the standard deviation in different ways
sd(mydata)  # Standard deviation without specifying arguments
sd(x = mydata)  # Explicitly name the argument 'x'
sd(x = mydata, na.rm = FALSE)  # Calculate with na.rm set to FALSE
sd(na.rm = FALSE, x = mydata)  # Order of arguments switched
sd(na.rm = FALSE, mydata)  # Mixing named and unnamed arguments
```

Even though it's legal, It's not recommended to messing around with the order of the arguments too much since it can lead to some confusion.
...
lm(data = mydata, y \~ x, model = FALSE, subset = 1:100) lm(y \~ x, mydata, subset = 1:100, model = FALSE)
...
f \<- function(a, b = 1, c = 2, d = NULL) {    function body }
...
columnMean <- function(x, removeNA = TRUE) {
  nc <- ncol(x)
  means <- numeric(nc) # Initialize a numeric vector to store the column means
...
Many operations in R are vectorized keying the code more efficient, concise, and easier to read.
